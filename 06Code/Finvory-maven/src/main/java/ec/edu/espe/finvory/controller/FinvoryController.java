...
public class FinvoryController {
    ...
    private void handleEditProduct() {
        ...
        try {
            ...
            if (!newCost.isEmpty()) {
                ...
                if (cost >= 0) product.setBaseCostPrice(cost);
                else view.showError("El costo no puede ser negativo.");
            }
    ...
    private void handleReportsMenu() {
        ...
        while (running) {
            ...
            switch (option) {
                case 1 -> view.showDashboard(data.getTotalGrossDay(), data.getTotalGrossProfile());
                case 2 -> {
    ...
    private void handleGenerateQuote() {
        ...
        while (true) {
            ...
            int quantity = -1; 
            try { quantity = view.askQuantity(999999); } catch(Exception e) { quantity = 0; }
            if (quantity <= 0) continue;
        ...
        if (quote.getLines().isEmpty()) return;
    ...
    private Customer findCustomer(String format) {
        ...
        for (Customer c : data.getCustomers()) 
            if (format.equals(c.getIdentification()) || (c.getName()!=null && c.getName().toLowerCase().contains(format))) return c;
        return null;
    }
    private Supplier findSupplier(String id) {
        if (id == null) return null;
        for (Supplier supplier : data.getSuppliers()) if (id.equals(supplier.getId1())) return supplier;
        return null;
    }
    private CompanyAccount findCompanyByUsername(String usernameCompany) {
        if (usernameCompany == null) return null;
        for (CompanyAccount c : users.getCompanyAccounts()) if (usernameCompany.equals(c.getUsername())) return c;
        return null;
    }
    private PersonalAccount findPersonalByUsername(String username) {
        if (username == null) return null;
        for (PersonalAccount personalAccount : users.getPersonalAccounts()) if (username.equals(personalAccount.getUsername())) return personalAccount;
        return null;
    }
}